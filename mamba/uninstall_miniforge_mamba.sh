...
# ───────────────────────────── Backup Functions ────────────────────────────
create_backup() {
    [[ $NO_BACKUP == true ]] && { print_info "Skipping backup (--no-backup specified)"; return 0; }
    [[ $DRY_RUN == true ]] && { print_dry_run "Would create backup at: $BACKUP_DIR"; return 0; }
...
run_anaconda_clean() {
    ...
    if exists conda; then
        ...
        execute_or_dry_run "Installing anaconda-clean" conda install anaconda-clean --yes 2>/dev/null || true
...
remove_configuration() {
    [[ $KEEP_CONFIG == true ]] && { print_info "Keeping configuration files (--keep-config specified)"; return; }
...
remove_cache() {
    [[ $KEEP_CACHE == true ]] && { print_info "Keeping cache directories (--keep-cache specified)"; return; }
...
main() {
    ...
    if verify_removal; then
        ...
        print_error "There was a problem with the uninstalling process. Please see log file."
    fi
}
